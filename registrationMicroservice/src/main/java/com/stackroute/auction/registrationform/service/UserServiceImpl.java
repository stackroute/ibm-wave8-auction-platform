...
public class UserServiceImpl implements UserService{
    ...
    @Override
    public List<List<RentItems>> getItems() {

        List<List<RentItems>> rentItems=new ArrayList<>();
        List<User> userList=userRepository.findAll();
        for(int i=0;i<userList.size();i++)
        {
            User user1=userList.get(i);
            rentItems.add(user1.getRentItems());
    ...
    @Override
    public List<RentItems> getItemByCategory(Category category)
    {
        List<List<RentItems>> allItems=getItems();
        List<RentItems> rentItems1=new ArrayList<>();
        for(int i=0; i<allItems.size(); i++) {
            for(int j=0; i< allItems.get(i).size(); i++) {
                System.out.println(allItems.get(i).get(j).getItemCategory());
                if(allItems.get(i).get(j).getItemCategory().getName().equals(category.getName())) {
                    rentItems1.add(allItems.get(i).get(j));
                    System.out.println(rentItems1);
                }
            }
        }
        return rentItems1;
