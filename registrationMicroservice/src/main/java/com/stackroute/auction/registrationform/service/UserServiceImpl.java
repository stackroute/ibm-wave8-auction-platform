...
public class UserServiceImpl implements UserService{
    ...
    @Override
    public List<List<RentItems>> getItemByCategory(String category)
    {
        List<List<RentItems>> allItems=getItems();
        List<List<RentItems>> rentItems1=new ArrayList<>();

        for(int i=0;i<allItems.size();i++)
        {
            int j=0;
            // System.out.println(allItems.get(i).get(j).getItemCategory());
            if(allItems.get(i).get(j).getItemCategory().equals(category))
            {
                rentItems1.add(allItems.get(i));
            }
        }
        return rentItems1;
    }
